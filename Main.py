...
def randomly_create_objects(number_of_objects, reward, xyDimension, reward_map=""):
    xSize = xyDimension[0]
    ySize = xyDimension[1]
...
def showPotentialAction(environment):
    cv2.imshow('image', cv2.resize(environment.getRepresentation(
        showAgent=True, scaleEnvironment=True), (200, 200), interpolation=cv2.INTER_NEAREST))
    cv2.waitKey(0)
...
def test_agent(directory):
    agent = linear.Linear(
        0.1, 0.9, ACTION_INFO, WORLD_SIZE, weight_scheme="ZERO", learning_rate=0.01
    )
